...
with raw_data as (
  ...
  , arrays as (
    select
      ...
      {{ snowplow_attribution.path_transformation('unique_path') }} as unique_path,
      {{ snowplow_attribution.path_transformation('frequency_path', '') }} as frequency_path,
      {{ snowplow_attribution.path_transformation('exposure_path', '') }} as exposure_path,
      {{ snowplow_attribution.path_transformation('first_path') }} as first_path,
      {{ snowplow_attribution.path_transformation('remove_if_not_all', 'Direct') }} as remove_if_not_all,
      {{ snowplow_attribution.path_transformation('remove_if_last_and_not_all', 'Direct') }} as remove_if_last_and_not_all
