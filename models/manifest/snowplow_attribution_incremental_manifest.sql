...
{{ config(
    ...
    meta={'upsert_date_key': 'processed_at', 'snowplow_optimize': true}
) }}
...
with prep as (
  ...
  select
    ...
    '{{ var("snowplow__conversion_hosts", [])|join(',') }}' as conversion_hosts,
    ...
    '{{ var("snowplow__conversion_hosts", [])|join(',') }}'  as channels_to_exclude,
    '{{ var("snowplow__conversion_hosts", [])|join(',') }}' as channels_to_include,
    '{{ var("snowplow__conversion_hosts", [])|join(',') }}' as campaigns_to_exclude,
    '{{ var("snowplow__conversion_hosts", [])|join(',') }}' as campaigns_to_include,
    '{{ var("snowplow__conversion_clause")|replace("'", "\\'") }}' as conversion_clause,
